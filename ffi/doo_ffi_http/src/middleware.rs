...
pub extern "C" fn jwt_middleware_handler(req: *const DooRequest, next: DooNextFn) -> *mut DooResult {
    if req.is_null() {
...
pub extern "C" fn cors_middleware_handler(req: *const DooRequest, next: DooNextFn) -> *mut DooResult {
    if req.is_null() {
        return next(req);
    }
...
pub extern "C" fn ratelimit_middleware_handler(req: *const DooRequest, next: DooNextFn) -> *mut DooResult {
    if req.is_null() {
        return next(req);
    }
