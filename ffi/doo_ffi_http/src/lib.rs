...
// These macros provide zero-overhead on success path (just a function call wrapper).

/// Wrap an extern "C" fn that returns *mut DooResult.
/// On panic → returns a 500 error DooResult.
macro_rules! ffi_safe_result {
    ($body:expr) => {{
        match catch_unwind(AssertUnwindSafe(|| $body)) {
            Ok(r) => r,
            Err(_) => make_err_http(500, "Internal server error (panic)"),
        }
    }};
}
...
/// On panic → silently absorb (logged by default in debug).
...
// string_to_c_local removed — use string_to_c from helpers (single source of truth)
...
pub extern "C" fn doohttp_create_response_from_result(
    ...
    match result {
        Ok(v) => v,
        Err(_) => {
            eprintln!("[FFI PANIC] doohttp_create_response_from_result");
            std::ptr::null_mut()
        }
    }
}
...
// ============================================================================
// UTILITY FUNCTIONS — using doo_ffi_core::DooResult (single source of truth)
// ============================================================================
...
pub extern "C" fn doo_ws_active_connections(_server: *const c_void) -> i64 {
    ffi_safe_i64!({ ws::get_conn_registry().count() as i64 })
}
