...
pub fn compile_project(opts: CompileOptions) -> Result<CompileResult, String> {
    ...
    let input_path = if opts.input_path.is_file() {
    ...
    } else if opts.input_path.exists() && opts.input_path.is_dir() {
        // It's a directory - look for main.doo
        ...
        // Path doesn't exist
        return Err(format!(
            "Error: path not found: {}\nPlease provide a valid .doo file or project directory",
            opts.input_path.display()
        ));
    };
    ...
    crate::doo_mir_debug!(
        "Starting MIR build: {} total nodes ({} imported structs, {} imported functions)",
        all_nodes.len(),
        analyzer.imported_structs.len(),
        analyzer.imported_functions.len()
    );
    ...
    crate::doo_mir_debug!(
        "MIR complete: {} functions, {} globals",
        mir_builder.program.functions.len(),
        mir_builder.program.globals.len()
    );
