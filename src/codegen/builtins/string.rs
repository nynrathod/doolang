...
impl<'ctx> CodeGen<'ctx> {
    pub fn generate_string_method(
        ...
        match method {
            ...
            "split" => {
                let str_ptr = object_val.into_pointer_value();
                let delimiter_ptr = self.resolve_value(&args[0]).into_pointer_value();

                // For now, implement simple split - count occurrences and create array
                // This is a simplified version that creates an array of string pointers
                ...
                // Allocate array (simplified - allocate max possible size)
                let malloc_fn = self.module.get_function("malloc").unwrap_or_else(|| {
                    let fn_type = self
                        .context
                        .i8_type()
                ...
                // Allocate heap for array (header + data)
                ...
                let heap_ptr = self
                    .builder
                    .build_call(malloc_fn, &[total_size.into()], "split_array")
                    .unwrap()
                    .try_as_basic_value()
                    .left()
                    .unwrap()
                ...
                // Full implementation would require complex string tokenization
                ...
                self.array_metadata.insert(dest.to_string(), metadata);
                Some(data_ptr.into())
            }
            "indexOf" => {
                let str_ptr = object_val.into_pointer_value();
