            ...
            MirInstrKind::IsOk { dest, value } => {
                ...
                if let Some(result_struct) = load_result_struct(ctx, result_val) {
                    ...
                    if std::env::var("DOO_DEBUG").is_ok() {
                        ...
                        let _ = ctx.builder.build_call(printf, &[fmt.into(), tag.into()], "debug_print");
                    }
...
fn ffi_type_to_llvm<'ctx>(
    ...
    match type_str {
        ...
        "simple_result" => {
            let struct_ty = ctx.context.struct_type(&[ctx.i64_type().into(), ctx.i64_type().into()], false);
            Some(struct_ty.into())
        }
    ...
    match expected_type {
        ...
        Some("ptr") => {
            ...
            if val.is_int_value() {
                ...
                let alloca = ctx.builder.build_alloca(struct_val.get_type(), "enum_box").unwrap();
...
fn try_convert_enum_to_json_string<'ctx>(
    ...
    let variants: Vec<(String, u32)> = match &type_info.kind {
        doo_core::types::TypeKind::Enum { variants, .. } => {
            variants.iter()
                .enumerate()
    ...
    let default_block = ctx.context.append_basic_block(target_fn, "enum_json_default");
    ...
    let ptr_type = ctx.ptr_type();
    let mut incoming_vals: Vec<(BasicValueEnum<'ctx>, inkwell::basic_block::BasicBlock<'ctx>)> = Vec::new();
    ...
    for (variant_name, variant_idx) in &variants {
        let case_block = ctx.context.append_basic_block(target_fn, &format!("enum_case_{}", variant_name));
        ...
        cases.push((ctx.context.i32_type().const_int(*variant_idx as u64, false), case_block));
...
fn try_convert_enum_array_to_json_string<'ctx>(
    ...
    if let Some(info) = &type_info {
        if let doo_core::types::TypeKind::Enum { name, variants, .. } = &info.kind {
            ...
            let serialize_fn = ctx.module.get_function("doo_db_serialize_enum_array")
                .unwrap_or_else(|| {
                    let ptr_type = ctx.ptr_type();
                    let i32_type = ctx.i32_type();
                    let fn_type = ptr_type.fn_type(&[ptr_type.into(), ptr_type.into(), i32_type.into()], false);
                    ctx.module.add_function("doo_db_serialize_enum_array", fn_type, None)
                });
...
fn try_convert_mixed_enum_array_to_json_string<'ctx>(
    ...
    for temp_name in &element_temps {
        if let Some(enum_name) = ctx.temp_struct_types.get(temp_name) {
            ...
            if let doo_core::types::TypeKind::Enum { variants, .. } = &type_info.kind {
                let variant_list: Vec<(String, u32)> = variants.iter()
                    .enumerate()
                    .map(|(i, (name, _))| (name.clone(), i as u32))
                    .collect();
    ...
    let buffer = ctx.builder.build_array_alloca(i8_type, buffer_size, "mixed_json_buf").ok()?;
    ...
    ctx.builder.build_call(sprintf, &[buffer.into(), open_bracket.into()], "").ok();
    
    // For each element, generate switch-case to append "variant"
    for (elem_idx, (temp_name, (enum_name, variants))) in element_temps.iter().zip(enum_infos.iter()).enumerate() {
        // Get the current buffer position
        let current_len = ctx.builder
        ...
        if elem_idx > 0 {
            ...
            ctx.builder.build_call(sprintf, &[write_pos.into(), comma_fmt.into()], "").ok();
        ...
        let current_block = ctx.builder.get_insert_block()?;
        let target_fn = current_block.get_parent()?;
        let merge_block = ctx.context.append_basic_block(target_fn, &format!("mixed_merge_{}", elem_idx));
        let default_block = ctx.context.append_basic_block(target_fn, &format!("mixed_default_{}", elem_idx));
        
        // Default: write "Unknown"
        ctx.builder.position_at_end(default_block);
        let unknown_fmt = ctx.const_string("\"Unknown\"");
        ctx.builder.build_call(sprintf, &[write_pos.into(), unknown_fmt.into()], "").ok();
        ctx.builder.build_unconditional_branch(merge_block).ok();
        
        // Cases for each variant
        let mut cases = Vec::new();
        for (variant_name, variant_idx) in variants {
            let case_block = ctx.context.append_basic_block(target_fn, &format!("mixed_case_{}_{}", elem_idx, variant_name));
            ctx.builder.position_at_end(case_block);
            
            let variant_fmt = ctx.const_string(&format!("\"{}\"", variant_name));
            ctx.builder.build_call(sprintf, &[write_pos.into(), variant_fmt.into()], "").ok();
            ctx.builder.build_unconditional_branch(merge_block).ok();
    ...
    ctx.builder.build_call(sprintf, &[write_pos.into(), close_bracket.into()], "").ok();
