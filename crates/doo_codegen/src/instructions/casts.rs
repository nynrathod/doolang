            ...
            MirInstrKind::Cast { dest, value, to_type } => {
                ...
                let result = if target_type_id == builtin::STR {
                    ...
                    if target_type_id == builtin::INT {
                        let cast_fn = ctx.module.get_function(ffi_names::DOO_RUNTIME_CAST_STR_TO_INT).unwrap_or_else(|| {
                            let i64_ty = ctx.context.i64_type();
                            let ptr_ty = ctx.context.i8_type().ptr_type(inkwell::AddressSpace::default());
                            let fn_ty = i64_ty.fn_type(&[ptr_ty.into()], false);
                            ctx.module.add_function(ffi_names::DOO_RUNTIME_CAST_STR_TO_INT, fn_ty, None)
                        });
                        
                        ctx.builder.build_call(cast_fn, &[source_val.into()], "cast_str_int")
                            .ok()?
                            .try_as_basic_value()
                            .left()?
                            .into()
                    } else if target_type_id == builtin::FLOAT {
                        // We don't have DOO_RUNTIME_CAST_STR_TO_FLOAT in ffi_names yet?
                        // Let's rely on atof/strtod for now or declare it.
                        // Ideally we add it to ffi_names.
                        // Assuming "atof" exists in libc.
                        let atof = ctx.module.get_function("atof").unwrap_or_else(|| {
                            let f64_ty = ctx.context.f64_type();
                            let ptr_ty = ctx.context.i8_type().ptr_type(inkwell::AddressSpace::default());
                            let fn_ty = f64_ty.fn_type(&[ptr_ty.into()], false);
                            ctx.module.add_function("atof", fn_ty, None)
                        });
                        ctx.builder.build_call(atof, &[source_val.into()], "cast_str_float")
                            .ok()?
                            .try_as_basic_value()
                            .left()?
                            .into()
                        ...
                        if target_llvm_ty.is_pointer_type() {
                            ...
                            source_val // No-op if types match or unknown
                        }
                    }
                    ...
                    if target_type_id == builtin::FLOAT {
                        // Int -> Float: signed int to double
                        ctx.builder.build_signed_int_to_float(int_val, ctx.f64_type(), "cast")
                            .ok()?
                            .into()
                        ...
                        ctx.builder.build_int_compare(
                            inkwell::IntPredicate::NE, int_val, zero, "cast"
                        ).ok()?.into()
                    } else {
                        // Int -> Int: extend/truncate as needed
                        let target_bits = 64; // Default to i64
                        let source_bits = int_val.get_type().get_bit_width();
                        
                        if target_bits > source_bits {
                            ctx.builder.build_int_s_extend(int_val, ctx.i64_type(), "cast")
                                .ok()?.into()
                        } else if target_bits < source_bits {
                            ctx.builder.build_int_truncate(int_val, ctx.i64_type(), "cast")
                                .ok()?.into()
                        } else {
                            source_val
                        }
                    ...
                    if target_type_id == builtin::INT {
                        ...
                        ctx.builder.build_float_to_signed_int(float_val, ctx.i64_type(), "cast")
                            .ok()?
                            .into()
                    } else if target_type_id == builtin::BOOL {
                        ...
                        ctx.builder.build_float_compare(
                            inkwell::FloatPredicate::ONE, float_val, zero, "cast"
                        ).ok()?.into()
                    } else {
...
fn emit_string_from_value<'ctx>(
    ...
    let snprintf = ctx.module.get_function(ffi_names::SNPRINTF).unwrap_or_else(|| {
    ...
    let null_buf = ctx.context.i8_type().ptr_type(inkwell::AddressSpace::default()).const_null();
    let zero_len = ctx.context.i64_type().const_zero();

    // Call snprintf(null, 0, fmt, val) to get length
    let mut call_args = vec![null_buf.into(), zero_len.into(), fmt.into()];
    ...
    let len_i64 = ctx.builder.build_int_z_extend(len_i32, ctx.i64_type(), "len64").ok()?;
    let size = ctx.builder.build_int_add(len_i64, ctx.i64_type().const_int(1, false), "size").ok()?;
    ...
    // Actual format
    let mut print_args = vec![buf.into(), size.into(), fmt.into()];
