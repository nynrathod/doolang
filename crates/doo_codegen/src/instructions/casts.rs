            ...
            MirInstrKind::Cast { dest, value, to_type } => {
                ...
                let result: Option<BasicValueEnum<'ctx>> = if target_type_id == builtin::STR {
                    ...
                    if target_type_id == builtin::INT {
                        let cast_fn = ctx.module.get_function(ffi_names::DOO_CAST_STR_TO_INT).unwrap_or_else(|| {
                            let i64_ty = ctx.context.i64_type();
                            let ptr_ty = ctx.context.i8_type().ptr_type(inkwell::AddressSpace::default());
                            let fn_ty = i64_ty.fn_type(&[ptr_ty.into()], false);
                            ctx.module.add_function(ffi_names::DOO_CAST_STR_TO_INT, fn_ty, None)
                        });
                        
                        Some(ctx.builder.build_call(cast_fn, &[source_val.into()], "cast_str_int")
                            .ok()?
                            .try_as_basic_value()
                            .left()?
                            .into())
                        ...
                        Some(ctx.builder.build_call(atof, &[source_val.into()], "cast_str_float")
                            .ok()?
                            .try_as_basic_value()
                            .left()?
                            .into())
                        ...
                        if target_llvm_ty.is_pointer_type() {
                             Some(ctx.builder.build_pointer_cast(
                    ...
                    if target_type_id == builtin::FLOAT {
                        ...
                        Some(ctx.builder.build_signed_int_to_float(int_val, ctx.f64_type(), "cast")
                            .ok()?
                            .into())
                        ...
                        if target_bits > source_bits {
                            Some(ctx.builder.build_int_s_extend(int_val, ctx.i64_type(), "cast")
                                .ok()?.into())
                        } else if target_bits < source_bits {
                            Some(ctx.builder.build_int_truncate(int_val, ctx.i64_type(), "cast")
                                .ok()?.into())
                        } else {
                            Some(source_val)
                        }
                    ...
                    if target_type_id == builtin::INT {
                        ...
                        Some(ctx.builder.build_float_to_signed_int(float_val, ctx.i64_type(), "cast")
                            .ok()?
                            .into())
                    } else if target_type_id == builtin::BOOL {
                        // Float -> Bool: compare != 0.0
                        let zero = ctx.f64_type().const_float(0.0);
                        Some(ctx.builder.build_float_compare(
...
fn emit_string_from_value<'ctx>(
    ...
    let mut call_args: Vec<BasicMetadataValueEnum> = vec![null_buf.into(), zero_len.into(), fmt.into()];
