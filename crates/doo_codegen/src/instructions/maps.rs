...
use crate::utils::{operand_to_value, emit_eq, default_for_type};
...
impl<'ctx> InstructionHandler<'ctx> for MapHandler {
    ...
    fn emit(
        ...
        match &instr.kind {
            MirInstrKind::MapCreate { dest, entries, key_type, val_type } => {
                ...
                let data_ptr = alloc_with_header(ctx, len_i32, pair_ty.into(), "map")?;
                ...
                for (i, (k, v)) in entries.iter().enumerate() {
                    let Some(kv) = operand_to_value(ctx, k) else { continue; };
                    let Some(vv) = operand_to_value(ctx, v) else { continue; };
            ...
            MirInstrKind::MapGet { dest, map, key, key_type, val_type } => {
                ...
                ctx.builder.build_store(idx_alloca, ctx.i64_type().const_zero()).ok();
                ...
                let val_ptr = ctx.builder.build_struct_gep(pair_ty, pair_ptr, 1, "val_ptr").ok()?;
                let stored_val = ctx.builder.build_load(val_llvm, val_ptr, "stored_val").ok()?;
            ...
            MirInstrKind::MapHas { dest, map, key, key_type, val_type } => {
                let mapv = operand_to_value(ctx, map)?;
                let keyv = operand_to_value(ctx, key)?;
                if !mapv.is_pointer_value() {
                    return None;
                ...
                ctx.builder.build_store(res_alloca, ctx.bool_type().const_zero()).ok();
                ...
                ctx.builder.position_at_end(found_bb);
                ctx.builder.build_store(res_alloca, ctx.bool_type().const_int(1, false)).ok();
                ctx.builder.build_unconditional_branch(end_bb).ok()?;
            ...
            MirInstrKind::MapSet { map, key, value, key_type, val_type } => {
                let mapv = operand_to_value(ctx, map)?;
                let keyv = operand_to_value(ctx, key)?;
                ...
                let not_found_bb = ctx.context.append_basic_block(current_fn, "map_set_not_found");
                ...
                let cond = ctx
                    ...
                    .build_int_compare(IntPredicate::ULT, idx, len_i64, "cond")
                    .ok()?;
                ctx.builder.build_conditional_branch(cond, check_bb, not_found_bb).ok()?;
                ...
                let new_len_i64 = ctx.builder.build_int_z_extend(new_len_i32, ctx.i64_type(), "new_len_i64").ok()?;
                let data_bytes = ctx.builder.build_int_mul(new_len_i64, pair_size, "data_bytes").ok()?;
                let total = ctx.builder.build_int_add(ctx.i64_type().const_int(8, false), data_bytes, "total").ok()?;
                let new_header = ctx
                    .builder
                ...
                let new_base = ctx
                    ...
                    .ok()?;
                let append_idx = ctx.builder.build_int_z_extend(len_i32, ctx.i64_type(), "append_idx")
                    .ok()?;
                let new_pair_ptr = unsafe { ctx.builder.build_gep(pair_ty, new_base, &[append_idx], "new_pair") }.ok()?;
                let nk = ctx.builder.build_struct_gep(pair_ty, new_pair_ptr, 0, "new_key_ptr").ok()?;
                let nv = ctx.builder.build_struct_gep(pair_ty, new_pair_ptr, 1, "new_val_ptr").ok()?;
