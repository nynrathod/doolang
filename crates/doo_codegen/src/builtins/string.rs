...
impl StringBuiltins {
    ...
    fn emit_len<'ctx>(
        ...
        let len_i64 = ctx.builder
            .build_call(strlen, &[str_ptr.into()], "strlen")
            .ok()?
            .try_as_basic_value()
            .left()?
            .into_int_value();
        let len_i32 = ctx.builder
    ...
    fn emit_char_at<'ctx>(
        ...
        let idx_i64 = ctx.builder.build_int_z_extend(index, ctx.context.i64_type(), "idx").ok()?;
        let char_ptr = unsafe {
            ctx.builder.build_in_bounds_gep(ctx.context.i8_type(), str_ptr, &[idx_i64], "char_ptr").ok()?
        };
        let char_val = ctx.builder.build_load(ctx.context.i8_type(), char_ptr, "char").ok()?;
        
        ctx.builder.build_store(result_ptr, char_val).ok()?;
        let null_ptr = unsafe {
            ctx.builder.build_in_bounds_gep(ctx.context.i8_type(), result_ptr, 
                &[ctx.context.i64_type().const_int(1, false)], "null_ptr").ok()?
        };
        ctx.builder.build_store(null_ptr, ctx.context.i8_type().const_int(0, false)).ok()?;
    ...
    fn emit_substring<'ctx>(
        ...
        let len_i64 = ctx.builder.build_int_z_extend(len, ctx.context.i64_type(), "len64").ok()?;
        let size = ctx.builder.build_int_add(len_i64, ctx.context.i64_type().const_int(1, false), "size").ok()?;
        ...
        // Copy substring
        let start_i64 = ctx.builder.build_int_z_extend(start, ctx.context.i64_type(), "start64").ok()?;
        let src_ptr = unsafe {
            ctx.builder.build_in_bounds_gep(ctx.context.i8_type(), str_ptr, &[start_i64], "src").ok()?
        };
        ctx.builder.build_call(memcpy, &[result_ptr.into(), src_ptr.into(), len_i64.into()], "").ok()?;
        
        // Null terminate
        let null_ptr = unsafe {
            ctx.builder.build_in_bounds_gep(ctx.context.i8_type(), result_ptr, &[len_i64], "null_ptr").ok()?
        };
        ctx.builder.build_store(null_ptr, ctx.context.i8_type().const_int(0, false)).ok()?;
    ...
    fn emit_concat<'ctx>(
        ...
        // Get lengths
        let len1 = ctx.builder.build_call(strlen, &[str_ptr.into()], "len1").ok()?
            .try_as_basic_value().left()?.into_int_value();
        let len2 = ctx.builder.build_call(strlen, &[other_ptr.into()], "len2").ok()?
            .try_as_basic_value().left()?.into_int_value();
        
        // Allocate len1 + len2 + 1
        let total_len = ctx.builder.build_int_add(len1, len2, "total").ok()?;
        let size = ctx.builder.build_int_add(total_len, ctx.context.i64_type().const_int(1, false), "size").ok()?;
        ...
        // Copy first string
        ctx.builder.build_call(memcpy, &[result_ptr.into(), str_ptr.into(), len1.into()], "").ok()?;
        
        // Copy second string
        let dest2 = unsafe {
            ctx.builder.build_in_bounds_gep(ctx.context.i8_type(), result_ptr, &[len1], "dest2").ok()?
        };
        let len2_plus_null = ctx.builder.build_int_add(len2, ctx.context.i64_type().const_int(1, false), "len2p1").ok()?;
        ctx.builder.build_call(memcpy, &[dest2.into(), other_ptr.into(), len2_plus_null.into()], "").ok()?;
    ...
    fn emit_index_of<'ctx>(
        ...
        let str_int = ctx.builder.build_ptr_to_int(str_ptr, ctx.context.i64_type(), "str_int").ok()?;
        let found_int = ctx.builder.build_ptr_to_int(found_ptr, ctx.context.i64_type(), "found_int").ok()?;
        let diff = ctx.builder.build_int_sub(found_int, str_int, "diff").ok()?;
        let diff_i32 = ctx.builder.build_int_truncate(diff, ctx.context.i32_type(), "diff32").ok()?;
        
        let neg_one = ctx.context.i32_type().const_int((-1_i32) as u64, true);
        let result = ctx.builder.build_select(is_null, neg_one, diff_i32, "indexOf").ok()?;
    ...
    fn emit_case_convert<'ctx>(
        ...
        let len = ctx.builder.build_call(strlen, &[str_ptr.into()], "len").ok()?
            .try_as_basic_value().left()?.into_int_value();
        let size = ctx.builder.build_int_add(len, ctx.context.i64_type().const_int(1, false), "size").ok()?;
        ...
        let cmp = ctx.builder.build_int_compare(IntPredicate::ULT, idx, len, "cmp").ok()?;
        ctx.builder.build_conditional_branch(cmp, body_bb, after_bb).ok()?;
        
        // Loop body
        ctx.builder.position_at_end(body_bb);
        let src_ptr = unsafe { ctx.builder.build_in_bounds_gep(ctx.context.i8_type(), str_ptr, &[idx], "src").ok()? };
        let dst_ptr = unsafe { ctx.builder.build_in_bounds_gep(ctx.context.i8_type(), result_ptr, &[idx], "dst").ok()? };
        let char_val = ctx.builder.build_load(ctx.context.i8_type(), src_ptr, "char").ok()?.into_int_value();
        ...
        let is_in_range_start = ctx.builder.build_int_compare(
            IntPredicate::UGE, char_val, ctx.context.i8_type().const_int(range_start, false), "ge"
        ).ok()?;
        let is_in_range_end = ctx.builder.build_int_compare(
            IntPredicate::ULE, char_val, ctx.context.i8_type().const_int(range_end, false), "le"
        ).ok()?;
        let is_in_range = ctx.builder.build_and(is_in_range_start, is_in_range_end, "in_range").ok()?;
        
        let converted = if to_upper {
            ctx.builder.build_int_sub(char_val, ctx.context.i8_type().const_int(32, false), "conv").ok()?
        } else {
            ctx.builder.build_int_add(char_val, ctx.context.i8_type().const_int(32, false), "conv").ok()?
        };
        let final_char = ctx.builder.build_select(is_in_range, converted, char_val, "final").ok()?;
        ctx.builder.build_store(dst_ptr, final_char).ok()?;
        
        let next_idx = ctx.builder.build_int_add(idx, ctx.context.i64_type().const_int(1, false), "next").ok()?;
        ...
        let null_ptr = unsafe {
            ctx.builder.build_in_bounds_gep(ctx.context.i8_type(), result_ptr, &[len], "null").ok()?
        };
        ctx.builder.build_store(null_ptr, ctx.context.i8_type().const_int(0, false)).ok()?;
    ...
    fn emit_replace<'ctx>(
        ...
        let current_fn = ctx.builder.get_insert_block()?.get_parent()?;
        let not_found_bb = ctx.context.append_basic_block(current_fn, "replace_not_found");
        ...
        let res_alloca = ctx
            .builder
            .build_alloca(ctx.context.i8_type().ptr_type(AddressSpace::default()), "replace_res")
            .ok()?;
        ...
        let found_i = ctx
            ...
            .ok()?;
        let prefix_len = ctx.builder.build_int_sub(found_i, hay_i, "prefix_len").ok()?;
        let suffix_start = ctx.builder.build_int_add(prefix_len, old_len, "suffix_start").ok()?;
        let suffix_len = ctx.builder.build_int_sub(hay_len, suffix_start, "suffix_len").ok()?;

        let t1 = ctx
            .builder
            .build_int_add(prefix_len, new_len, "t1")
            .ok()?;
        let total_no_nul = ctx
            .builder
            .build_int_add(t1, suffix_len, "t2")
            .ok()?;
        let total = ctx
            .builder
            .build_int_add(total_no_nul, ctx.context.i64_type().const_int(1, false), "total")
            .ok()?;
        ...
        // copy prefix
        ctx.builder
            .build_call(memcpy, &[out_ptr.into(), str_ptr.into(), prefix_len.into()], "")
            .ok()?;
        // copy new
        let dst_new = unsafe {
            ctx.builder
                .build_in_bounds_gep(ctx.context.i8_type(), out_ptr, &[prefix_len], "dst_new")
        }
        .ok()?;
        ctx.builder
            .build_call(memcpy, &[dst_new.into(), new_ptr.into(), new_len.into()], "")
            .ok()?;
        // copy suffix
        let dst_off = ctx
            .builder
        ...
        let src_suffix = unsafe {
            ctx.builder
                .build_in_bounds_gep(ctx.context.i8_type(), str_ptr, &[suffix_start], "src_suf")
        }
        .ok()?;
        ctx.builder
            .build_call(memcpy, &[dst_suffix.into(), src_suffix.into(), suffix_len.into()], "")
            .ok()?;
        // nul
        let nul_ptr = unsafe {
            ctx.builder
        ...
        let res = ctx
            .builder
            .build_load(ctx.context.i8_type().ptr_type(AddressSpace::default()), res_alloca, "replace")
            .ok()?;
        Some(res)
    }
    ...
    fn emit_trim<'ctx>(
        ...
        // start scan
        let start_alloca = ctx.builder.build_alloca(ctx.context.i64_type(), "start").ok()?;
        ctx.builder
            .build_store(start_alloca, ctx.context.i64_type().const_zero())
            .ok()?;
        let start_loop = ctx.context.append_basic_block(current_fn, "trim_start_loop");
        let start_body = ctx.context.append_basic_block(current_fn, "trim_start_body");
        ...
        let next_s = ctx
            ...
            .ok()?;
        let cont_bb = ctx.context.append_basic_block(current_fn, "trim_start_cont");
        ctx.builder
        ...
        let end_alloca = ctx.builder.build_alloca(ctx.context.i64_type(), "end").ok()?;
        ...
        let end_idx = ctx
            ...
            .ok()?
            .into_int_value();
        let out_len = ctx.builder.build_int_sub(end_idx, start_idx, "out_len").ok()?;
        let size = ctx
            .builder
        ...
        .ok()?;
        ctx.builder
            .build_call(memcpy, &[out_ptr.into(), src_ptr.into(), out_len.into()], "")
            .ok()?;
        let nul_ptr = unsafe {
            ctx.builder
...
fn build_is_ws<'ctx>(
    ...
    let is_sp = ctx.builder.build_int_compare(IntPredicate::EQ, ch, sp, "is_sp").ok()?;
    let is_tab = ctx.builder.build_int_compare(IntPredicate::EQ, ch, tab, "is_tab").ok()?;
    let is_nl = ctx.builder.build_int_compare(IntPredicate::EQ, ch, nl, "is_nl").ok()?;
    let is_cr = ctx.builder.build_int_compare(IntPredicate::EQ, ch, cr, "is_cr").ok()?;
    ...
    fn emit_reverse<'ctx>(
        ...
        let rev_idx = ctx.builder.build_int_sub(len, ctx.context.i64_type().const_int(1, false), "len_m1").ok()?;
        let rev_idx = ctx.builder.build_int_sub(rev_idx, idx, "rev_idx").ok()?;
        let src_ptr = unsafe { ctx.builder.build_in_bounds_gep(ctx.context.i8_type(), str_ptr, &[rev_idx], "src").ok()? };
        let dst_ptr = unsafe { ctx.builder.build_in_bounds_gep(ctx.context.i8_type(), result_ptr, &[idx], "dst").ok()? };
        let char_val = ctx.builder.build_load(ctx.context.i8_type(), src_ptr, "char").ok()?;
    ...
    fn emit_starts_with<'ctx>(
        ...
        let cmp = ctx.builder
            .build_call(strncmp, &[str_ptr.into(), prefix_ptr.into(), prefix_len.into()], "cmp")
            .ok()?
            .try_as_basic_value()
            .left()?
            .into_int_value();
        ...
        let is_eq = ctx.builder.build_int_compare(
            IntPredicate::EQ, cmp, ctx.context.i32_type().const_int(0, false), "eq"
        ).ok()?;
    ...
    fn emit_ends_with<'ctx>(
        ...
        let offset = ctx.builder.build_int_sub(str_len, suffix_len, "offset").ok()?;
        
        let start_ptr = unsafe {
            ctx.builder.build_in_bounds_gep(ctx.context.i8_type(), str_ptr, &[offset], "start").ok()?
        };
        
        let cmp = ctx.builder
            .build_call(strncmp, &[start_ptr.into(), suffix_ptr.into(), suffix_len.into()], "cmp")
            .ok()?
            .try_as_basic_value()
            .left()?
            .into_int_value();
    ...
    fn emit_repeat<'ctx>(
        ...
        let n_i64 = ctx.builder.build_int_z_extend(n, ctx.context.i64_type(), "n64").ok()?;
        ...
        ctx.builder.position_at_end(loop_bb);
        let idx = ctx.builder.build_load(ctx.context.i64_type(), idx_alloca, "idx").ok()?.into_int_value();
        let cmp = ctx.builder.build_int_compare(IntPredicate::ULT, idx, n_i64, "cmp").ok()?;
        ctx.builder.build_conditional_branch(cmp, body_bb, after_bb).ok()?;
        
        ctx.builder.position_at_end(body_bb);
        let offset = ctx.builder.build_int_mul(idx, len, "off").ok()?;
        let dst = unsafe { ctx.builder.build_in_bounds_gep(ctx.context.i8_type(), result_ptr, &[offset], "dst").ok()? };
        ctx.builder.build_call(memcpy, &[dst.into(), str_ptr.into(), len.into()], "").ok()?;
        ...
        ctx.builder.position_at_end(after_bb);
        let null_ptr = unsafe {
            ctx.builder.build_in_bounds_gep(ctx.context.i8_type(), result_ptr, &[total_len], "null").ok()?
        };
        ctx.builder.build_store(null_ptr, ctx.context.i8_type().const_int(0, false)).ok()?;
    ...
    fn emit_char_code<'ctx>(
        ...
        let char_val = ctx.builder.build_load(ctx.context.i8_type(), str_ptr, "char").ok()?.into_int_value();
        let code = ctx.builder.build_int_z_extend(char_val, ctx.context.i32_type(), "code").ok()?;
        Some(code.into())
    }
    ...
    fn emit_count_substr<'ctx>(
        ...
        let needle_len = ctx.builder.build_call(strlen, &[needle_ptr.into()], "nlen").ok()?
            .try_as_basic_value().left()?.into_int_value();
        ...
        let ptr_alloca = ctx.builder.build_alloca(ctx.context.i8_type().ptr_type(AddressSpace::default()), "ptr").ok()?;
        let count_alloca = ctx.builder.build_alloca(ctx.context.i32_type(), "count").ok()?;
        ctx.builder.build_store(ptr_alloca, str_ptr).ok()?;
        ctx.builder.build_store(count_alloca, ctx.context.i32_type().const_zero()).ok()?;
        ctx.builder.build_unconditional_branch(loop_bb).ok()?;
        
        ctx.builder.position_at_end(loop_bb);
        let ptr_type = ctx.context.i8_type().ptr_type(AddressSpace::default());
        let curr_ptr = ctx.builder.build_load(ptr_type, ptr_alloca, "curr").ok()?.into_pointer_value();
        let found = ctx.builder.build_call(strstr, &[curr_ptr.into(), needle_ptr.into()], "found").ok()?
            .try_as_basic_value().left()?.into_pointer_value();
        let is_null = ctx.builder.build_is_null(found, "is_null").ok()?;
        ctx.builder.build_conditional_branch(is_null, after_bb, body_bb).ok()?;
        
        ctx.builder.position_at_end(body_bb);
        let count = ctx.builder.build_load(ctx.context.i32_type(), count_alloca, "cnt").ok()?.into_int_value();
        let new_count = ctx.builder.build_int_add(count, ctx.context.i32_type().const_int(1, false), "new_cnt").ok()?;
        ctx.builder.build_store(count_alloca, new_count).ok()?;
        
        // Move pointer past the found occurrence
        let next_ptr = unsafe {
            ctx.builder.build_in_bounds_gep(ctx.context.i8_type(), found, &[needle_len], "next").ok()?
        };
        ctx.builder.build_store(ptr_alloca, next_ptr).ok()?;
        ctx.builder.build_unconditional_branch(loop_bb).ok()?;
        
        ctx.builder.position_at_end(after_bb);
        let final_count = ctx.builder.build_load(ctx.context.i32_type(), count_alloca, "final").ok()?;
...
fn get_or_declare_strstr<'ctx>(ctx: &CodegenContext<'ctx>) -> inkwell::values::FunctionValue<'ctx> {
    ctx.module.get_function(ffi_names::STRSTR).unwrap_or_else(|| {
        let ptr_type = ctx.context.i8_type().ptr_type(AddressSpace::default());
...
fn get_or_declare_strncmp<'ctx>(ctx: &CodegenContext<'ctx>) -> inkwell::values::FunctionValue<'ctx> {
    ctx.module.get_function(ffi_names::STRNCMP).unwrap_or_else(|| {
        let ptr_type = ctx.context.i8_type().ptr_type(AddressSpace::default());
        let fn_type = ctx.context.i32_type().fn_type(
            &[ptr_type.into(), ptr_type.into(), ctx.context.i64_type().into()],
            false
        );
