...
const toCopy = ["out", "syntaxes", "node_modules", "language-configuration.json", "package.json", "readme.md"];
...
if (fs.existsSync(registryPath)) {
  try {
    ...
    const idx = registry.findIndex((e) => e.identifier && e.identifier.id === extId);
    const locationPath = targetDir.replace(/\\/g, "/").replace(/^([A-Z]):/, (_, d) => `/${d.toLowerCase()}:`);
    const entry = {
      identifier: { id: extId },
      version: pkg.version,
...
console.log(`  Reload VS Code (Ctrl+Shift+P → "Developer: Reload Window") to activate.\n`);
